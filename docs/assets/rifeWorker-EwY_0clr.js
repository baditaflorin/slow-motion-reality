...
* =============================================================================
*/
let vd,yd=[],bd=``;self.onmessage=async e=>{try{if(e.data.type===`init`){vd=await Se.create(e.data.model,{executionProviders:[`webgpu`,`wasm`],graphOptimizationLevel:`all`}),yd=vd.inputNames,bd=vd.outputNames[0],self.postMessage({type:`ready`,modelName:e.data.modelName});return}if(!vd){self.postMessage({type:`error`,error:`RIFE model is not loaded.`});return}let t=await xd(e.data.first,e.data.second,e.data.alpha);self.postMessage({type:`frame`,bitmap:t},[t])}catch(e){self.postMessage({type:`error`,error:e instanceof Error?e.message:String(e)})}};async function xd(e,t,n){let r=Math.min(e.width,t.width,512),i=Math.min(e.height,t.height,288),a=await Sd(e,r,i),o=await Sd(t,r,i),s=new fe(`float32`,Float32Array.from([n]),[1]),c={};c[yd[0]??`I0`]=a,c[yd[1]??`I1`]=o,yd[2]&&(c[yd[2]]=s);let l=(await vd.run(c))[bd];return Cd(l,r,i)}async function Sd(e,t,n){let r=new OffscreenCanvas(t,n).getContext(`2d`);if(!r)throw Error(`OffscreenCanvas 2D context is unavailable.`);r.drawImage(e,0,0,t,n);let i=r.getImageData(0,0,t,n),a=new Float32Array(3*n*t);for(let e=0;e<t*n;e+=1)a[e]=i.data[e*4]/255,a[n*t+e]=i.data[e*4+1]/255,a[2*n*t+e]=i.data[e*4+2]/255;return new fe(`float32`,a,[1,3,n,t])}function Cd(e,t,n){let r=e.data,i=new OffscreenCanvas(t,n),a=i.getContext(`2d`);if(!a)throw Error(`OffscreenCanvas 2D context is unavailable.`);let o=a.createImageData(t,n);for(let e=0;e<t*n;e+=1)o.data[e*4]=wd(r[e]*255),o.data[e*4+1]=wd(r[n*t+e]*255),o.data[e*4+2]=wd(r[2*n*t+e]*255),o.data[e*4+3]=255;return a.putImageData(o,0,0),createImageBitmap(i)}function wd(e){return Math.max(0,Math.min(255,Math.round(e)))}})();